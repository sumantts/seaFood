                ...
                <form method="POST" class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="col-lg-4">
                            ...
                            <select id="exportHeader" required="" multiple="" data-placeholder="-- Select Report Header --" name="header[]" class="form-control select2">   
                                <option value="p_scientific_name">p_scientific_name</option>
                                <?php 
                                ...
                                ?>
                                    ...
                                    <?php if($value != 'product_id' && $value != 'od_id' && $value != 'offer_id' && $value != 'user_id' && $value != 'created_date' && $value != 'modified_date' && $value != 'status' && $value != 'cartons_offered' && $value != 'quantity_offered'){ ?>
                                        ...
                                        <?php if($value == 'product_line'){ ?>
                                            <option value="product_line">product_line_po</option>
                                        <?php }else{ ?>
                        ...
                        <div class="col-lg-4">
                            <label class="text-danger">Type (PO/SC)*</label><br>
                            <select id="type" required="" name="type" class="form-control select2">
                            		<option value="">-- Select Type --</option>
                                    <option value="PO">Purchase Order</option>
                                    <option value="SC">Sale Contract</option>
                            </select>
                        </div>
                    ...
                    <div class="panel-body">
                        <table id="existing_templates_table" class="table table-responsive table-bordered" style="width:100%;">
                            ...
                            <tbody>
                                <?php
                                $iter = 0;
                                $sc_template = $this->db->get('sc_template')->result();
                                foreach ($sc_template as $et) {
                                ?>
                                    <tr>
                                        <td><?= ++$iter ?></td>
                                        ...
                                        <td>
                                            <button data-vt_id="<?=$et->sc_template_id?>" type="button" class="btn btn-danger delete">Delete</button>
                                            <button data-vt_id="<?=$et->sc_template_id?>" type="button" class="btn btn-warning copy">Copy</button>
                                        </td>
                                    </tr>
                                <?php
                                    }
...
<script>
    ...
    $(document).on('click', '.delete', function(){
        ...
        if(confirm("Are You Sure? This Process Can\'t be Undo.")){
            ...
            $.ajax({
                url: "<?= base_url('admin/ajax_delete_acc_template')?>/"+$vt_id,
                dataType: 'json',
                type: 'POST',
    ...
    $(document).on('click', '.copy', function(){
        ...
        var type = $.trim($(this).closest('td').prev('td').text());
        var filter_val = ($("#type option:contains('"+type+"')").val());
        $("#type").select2("val", filter_val);
